...
function displayPort(): DeviceTopologyPort {
  return {
    ...
    display: {
      ...
      outputTechnology: "displayport-external",
      ...
      hdrFormats: "HDR10 / PQ",
      displayTechnology: { domain: 5, code: 78, args: ["displayport"] },
...
function usbPort(options: {
  ...
  return {
    ...
    protocol: "USB 3.x",
    speed: options.connected ? "USB 3.x SuperSpeed / 5Gbps+" : null,
...
function usbConnection(options: {
  ...
  return {
    ...
    connectionStatus: options.connected ? { domain: 5, code: 47, args: [] } : { domain: 5, code: 46, args: [] },
    negotiatedSpeed: options.connected ? "USB 3.x SuperSpeed / 5Gbps+" : null,
