...
public sealed partial class OptimizationA2Service
{
    private static string? ResolveTargetDisabledReason(OptimizationReportItem report)
    {
        ...
        {
            return "A2 极限增强第一版只处理软件级目标。";
        }

        if (report.Type == OptimizationReportTypes.DiskPressure)
        {
            return "磁盘空间报告不属于 A2 极限增强动作。";
        }
        ...
        {
            SoftwareKinds.Game => null,
            SoftwareKinds.HighPerformance => "高性能专业软件不使用 A2 游戏极限增强，后续单独做专业增强模板。",
            SoftwareKinds.Adapted => "适配软件不套用不适配软件的外部增强等级。",
            SoftwareKinds.WindowsSystem or SoftwareKinds.WindowsComponent or SoftwareKinds.WindowsService => "Windows 系统目标不允许 A2 极限增强。",
            SoftwareKinds.Unattributed => "未归属进程不能直接应用软件级极限增强。",
            _ => "A2 极限增强只面向游戏或用户明确指定的极限性能目标。"
        };
    }
    ...
    private static int ActionOrder(string kind)
    {
        return kind switch
        {
            OptimizationA2ActionKinds.RaiseProcessPriority => 0,
            OptimizationA2ActionKinds.DisableExecutionSpeedThrottling => 1,
            OptimizationA2ActionKinds.MoveTargetAffinity => 2,
            _ => 99
        };
    }
