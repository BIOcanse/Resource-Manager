...
public static partial class MetricCatalog
{
    public static IReadOnlyList<MetricDefinition> FromSnapshot(HardwareMetricSnapshot snapshot)
    {
        ...
        var definitions = new List<MetricDefinition>
        {
            ...
            new("memory.usage", "内存占用", "Memory", MetricUnits.Bytes, "main", snapshot.Memory.HardwareDescription),
        ...
        AddIfAvailable(definitions, snapshot.Items, "virtualMemory.usage", "虚拟内存占用", "Virtual Memory", MetricUnits.Bytes, "main", snapshot.VirtualMemory.Detail);
