...
function decodeUsbConnection(value: unknown, path: string): DeviceTopologyUsbConnection {
  ...
  return {
    ...
    connectionStatus: requireBackendMessage(record.connectionStatus, `${path}.connectionStatus`),
    negotiatedSpeed: requireNullable(record.negotiatedSpeed ?? null, `${path}.negotiatedSpeed`, requireString),
