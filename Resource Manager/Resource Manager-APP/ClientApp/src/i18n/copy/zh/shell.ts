export const zhShellCopy = {
  ...
  shell: {
    ...
    maximize: "最大化",
    closeWindow: "关闭",
    uptimeLoading: "运行时长加载中",
    uptimeStale: (uptime: string) => `${uptime} · 非实时`,
    uptimeUnavailable: "运行时长不可用",
    uptimeDisabled: "运行时长已停用",
    observationLoading: (label: string) => `${label}加载中`,
    observationStale: (label: string) => `${label}非实时`,
    observationUnavailable: (label: string) => `${label}不可用`,
    observationDisabled: (label: string) => `${label}已停用`
  }
};
