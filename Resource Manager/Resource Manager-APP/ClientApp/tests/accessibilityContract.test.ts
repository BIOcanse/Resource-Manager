...
const metricModal = readSource("components/MetricModal.tsx");
const manualSoftwareModal = readSource("components/ManualSoftwareModal.tsx");
const managementPage = readSource("components/ManagementPage.tsx");
...
assert.match(managementPage, /<nav class="management-tabs" aria-label="组件与软件分类">/);
...
assert.match(appShell, /<PageBoundary name="当前页面" resetKey=\{props\.activePage\(\)\}>/);
assert.match(appShell, /document\.title = `\$\{props\.pageTitle\(\)\} · 资源管理器`/);
...
assert.match(monitorPage, /<ObservationStateNotice[\s\S]*?label="指标目录"[\s\S]*?presentation="blocking-only"/);
...
assert.match(monitorPage, /aria-label="取消监控面板布局编辑"/);
assert.match(resourceBreakdown, /aria-label="资源占用条目编辑器"/);
assert.match(resourceTable, /aria-label="资源列表列编辑器"/);
assert.match(dashboard, /aria-label=\{`为\$\{slotLabel\(\)\}添加指标`\}/);
assert.match(dashboard, /aria-label=\{`更换\$\{slotLabel\(\)\}：\$\{metricLabel\(\)\}`\}/);
assert.match(dashboard, /aria-label=\{`删除\$\{slotLabel\(\)\}：\$\{metricLabel\(\)\}`\}/);
